 ...
 * Modified for multi-site fitness birth-death model (April., 2018)
 * Sites are not sorted by site patterns so that sites will always be in the same order as in the input alignment
 * All site patterns are also treated as unique so each site has its own pattern
 */
...
public class UnsortedAlignment extends Alignment {
     ...
     */
    @Deprecated
    public UnsortedAlignment(List<Sequence> sequences, Integer stateCount, String dataType) {
    ...
    private void calcPatterns(boolean log) {
        ...
        maxDataState = maxDataState + 1; // add one b/c indexing states from zero
        ...
        for (int j = 0; j < siteCount; j++) {
        	...
        	maxDataStates[j] = max + 1; // add one b/c indexing states from zero
        }

        // sort data
        SiteComparator comparator = new SiteComparator();
        //Arrays.sort(data, comparator); // sites are not sorted by site patterns
        ...
        for (int i = 1; i < siteCount; i++) {
            //if (usingTipLikelihoods || comparator.compare(data[i - 1], data[i]) != 0) { // consider all patterns, unique or not
