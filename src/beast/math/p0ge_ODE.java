...
public class p0ge_ODE implements FirstOrderDifferentialEquations {
	...
	// ARE THESE USED? IF YES, THEY SHOULD REFLECT MODEL IN FITNESS SPACE FOR P0
	...
	public double[] getP(double t, Boolean rhoSampling, Double[] rho){
		...
		// dimension should = fitnessModel.getFitSpaceStates()
		...
		if (!rhoSampling)
			...
			for (int i = 0; i<dimension; i++) {
				...
				y[i] = (1 - rho[Utils.index(T, times, intervals)]);    // since rho will not be indexed only by the time interval
...
//
//			p0_ODE p_ode = new p0_ODE(b,null, d,s,M, 2, new Double[]{1., 1.}, 1, new Double[]{0.});
...
//
//		p0_ODE p_ode = new p0_ODE(b,new Double[]{1.,1.}, d,s,M, 2, new Double[]{1., 1.}, 1, new Double[]{0.});
