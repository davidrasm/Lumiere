function handles = phytree_plot(tr,varargin)
% Plot a phylogenetic tree in SimMap format. Like Matlab's 'phytree_plot', but enables different branch colors.
...
if(isfield(tr, 'colorMap'))
    maxState = length(tr.colorMap);
    colormap(tr.colorMap);
    colorMatrix = tr.colorMap;
else
    maxState = max(allSegStates);
    ...
    case {'left','right'}
        ...
        for lin = 1:(lineages-1)
            ...
            for seg = 1: numSegs
                ...
                %lineageSegs.segStates{lin}(seg)
                h.BranchLines(lineCounter) = plot(lineageSegs.x{lin}(1:2,seg), Y(1:2,lin), 'color', colorMatrix(lineageSegs.segStates{lin}(seg),:), 'LineWidth', 2.0); lineCounter = lineCounter + 1;
            end
            % plot vertical line segments
            h.BranchLines(lineCounter) = plot(X(2:3,lin), Y(2:3,lin), 'color', colorMatrix(lineageSegs.segStates{lin}(numSegs),:), 'LineWidth', 2.0); lineCounter = lineCounter + 1;
            ...
            case {'left','right'}
                ...
                %h.LeafDots = plot(tr.x(leafIndex(tr.activeNodes(leafIndex))),...
                 ...
                 for i = 1:tr.numLeaves
                    ...
                    leafState = lineageSegs.segStates{i}(1);
...
% xTickTrueTimes = (0:10:100); %(ceil(trueRootTime):10:100);
% xTickAxisLocs = xTickTrueTimes - trueRootTime;
% xTickAxisStart = 0; %ceil(trueRootTime);
% xTickAxisLabels = (xTickAxisStart:10:100);

% For ebola
trueRootTime = 2015.82 - max(tr.x);
xTickTrueTimes = floor(trueRootTime):0.5:2016;
xTickAxisLocs = xTickTrueTimes - trueRootTime;
xTickAxisLabels = xTickTrueTimes;
...
%handleNow = gca;
%set(h.axes, 'Color', [0.4, 0.4, 0.4]) %set plot background to black
...
if (tr.colorBar)
    ...
    tickStep = 1;
    labelStep = 1;
    %set(hcbar, 'YTick', [1:tickStep:maxState+1]); %set YTicks on colorbar
    %set(hcbar, 'YTickLabel', [0:labelStep:maxState]); %set labels
    %ylabel(hcbar, 'State', 'FontSize',14)
    
    % For Ebola
    set(hcbar, 'YTick', linspace(0,1,5)); %set YTicks on colorbar
    %set(hcbar, 'YTickLabel', linspace(0.95,1.15,4)); % Should match min/max state used to discretize fitness
    set(hcbar, 'YTickLabel', [0.95, 1.0, 1.05, 1.1, 1.15]);
    ylabel(hcbar, 'Fitness', 'FontSize',14)
...
%     labelStep = maxK / 4;
%     set(hcbar, 'YTick', [1:tickStep:maxState+1]); %set YTicks on colorbar
%     set(hcbar, 'YTickLabel', [0:labelStep:maxK]); %set labels
%     ylabel(hcbar, 'Expected degree', 'FontSize',14)
% else
%     step = maxK / 4;
%     set(hcbar, 'YTick', [1:step:maxK+1]); %set YTicks on colorbar
%     set(hcbar, 'YTickLabel', [0:step:maxK]); %set labels
%     ylabel(hcbar, 'State', 'FontSize',14)
% end

%ylabel(hcbar, 'Degree', 'FontSize',14)
xlabel(h.axes, 'Time', 'FontSize', 12); %set xlabel
