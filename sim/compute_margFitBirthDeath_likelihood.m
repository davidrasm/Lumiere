function [L,phy] = compute_margFiBirthDeath_likelihood(params, phy, fitModel)
    ...
    for lin = phy.tipCount+1:phy.nodeCount
        ...
        if (length(currLine.children) > 1)
            ...
            if (params.multiplicativeFitness)
                ...
                child1_full_Beta = scaledBeta * dot(genotypeProbs,fitModel.genotypeFitness);
                [genotypeProbs] = get_genotypeProbs(norm_child2_pD);
                child2_full_Beta = scaledBeta * dot(genotypeProbs,fitModel.genotypeFitness);
    ...
    function [pD, full_pD] = solve_siteLineProbsD(currLine, pD, full_pD)
        ...
        while (nextIntTime > endTime)
            ...
            if (params.multiplicativeFitness)
                ...
                [genotypeProbs] = get_genotypeProbs(norm_pD);
                expBeta = scaledBeta * dot(genotypeProbs,fitModel.genotypeFitness);
