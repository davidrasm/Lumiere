function [L,loggedProbsD,loggedTimes] = compute_birthDeathLikelihood(params, phy, genoMap)
...
for lin = 1:phy.tipCount
    ...
    if (params.trackLineProbs && currLine.save)
       loggedProbsD = sample_pD; 
       loggedTimes = currTime;
    end
...
for lin = phy.tipCount+1:phy.nodeCount
    ...
    if (length(currLine.children) > 1)
        ...
        if (params.trackLineProbs && currLine.save)
            loggedProbsD = [loggedProbsD, parent_pD];
            loggedTimes = [loggedTimes, currTime];
        end
...
L = logL;
    ...
    function [pD] = solve_lineProbsD(currLine, pD)
        ...
        while (nextIntTime > endTime) % nextLineTime)
            ...
            if (params.trackLineProbs && currLine.save)
                loggedProbsD = [loggedProbsD, pD];
                loggedTimes = [loggedTimes, times(tx-1)];
            end
